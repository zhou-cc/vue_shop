    ...
    <el-card>
      ...
      <el-row class="cat_opt">
        <el-col>
          <span>选择商品分类：</span>
          <!-- 选择商品分类的级联选择框 -->
          <el-cascader expand-trigger="hover" v-model="selectedCateKeys" :options="cateList" @change="handleChange" :props="cateProps" clearable change-on-select>
            </el-cascader>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    showInput(row){
        ...
        // 让文本框自动获得焦点
        // nextTick 方法的作用，就是当页面上元素被重新渲染后才会执行回调函数中的代码
        this.$nextTick(_ => {
